...
def get_video_path(root_path, ends_list):
    video_files = []
    for root, dirs, files in os.walk(os.path.join(root_path, 'Test_files')):
        for file in files:
            if file.endswith(tuple(ends_list)):
                full_path = os.path.join(root, file)
                video_files.append(full_path)
    
    for root, dirs, files in os.walk(os.path.join(root_path, 'Train_files')):
        for file in files:
            if file.endswith(tuple(ends_list)):
                full_path = os.path.join(root, file)
                video_files.append(full_path)
...
def oulu_transfer_img(original_video_path, output_dir):
    for video_path in tqdm(original_video_path):
        directory, filename = os.path.split(video_path)
        parts = directory.split(os.sep)

        new_folder_path = os.path.join(output_dir, 'train' if parts[-1] == 'Train_files' else 'test', 'real' if filename.split('.')[0][-1] == '1' else 'fake')
        os.makedirs(new_folder_path, exist_ok=True)

        frames = extract_frames(video_path, (1 / 4, 3 / 4))
        base_filename = f"{filename.split('.')[0]}"
        if len(frames) == 2:
...
if __name__ == '__main__':
    original_video_path = get_video_path(r"/media/disk2/cailvpan/FaceAnti-spoofing/OULU-NPU", ['.avi'])

    oulu_transfer_img(original_video_path, r'/home/wanghaowei/cailvpan/frame/oulu')
