...
def msu_transfer_img(original_video_path, output_dir):

    with open(r'/media/disk2/cailvpan/FaceAnti-spoofing/MSU-MFSD/train_sub_list.txt') as f:
        train_ids = [k.strip() for k in f.readlines()]
    ...
    for video_path in tqdm(original_video_path):
        ...
        new_folder_path = os.path.join(output_dir, 'train' if filename.split('_')[1][-2:] in train_ids else 'test', 'real' if parts[-1] == 'real' else 'fake')
...
if __name__ == '__main__':
    original_video_path = get_video_path(r"/media/disk2/cailvpan/FaceAnti-spoofing/MSU-MFSD", ['.mp4', '.mov'])

    msu_transfer_img(original_video_path, r'/home/wanghaowei/cailvpan/frame/msu')
