...
def get_video_path(root_path, ends_list):
    video_files = []
    for root, dirs, files in os.walk(os.path.join(root_path, 'test')):
        for file in files:
...
def replay_transfer_img(original_video_path, output_dir):
    for video_path in tqdm(original_video_path):
        directory, filename = os.path.split(video_path)
        parts = directory.split(os.sep)

        new_folder_path = os.path.join(output_dir, 'train' if parts[6] == 'train' else 'test', 'real' if parts[-1] == 'real' else 'fake')
...
if __name__ == '__main__':
    original_video_path = get_video_path(r"/media/disk2/cailvpan/FaceAnti-spoofing/Replay-Attack", ['.mov'])

    replay_transfer_img(original_video_path, r'/home/wanghaowei/cailvpan/frame/replay')
